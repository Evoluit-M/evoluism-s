...
def compute_risk_score(r2: float, dw: float, pval: float) -> float:
    """
    Compute heuristic 0–100% spurious regression risk score.
    """
...
### ✅ Option 1 — use built-in demo data  
This instantly generates two independent trending random-walk series.

### ✅ Option 2 — upload your own CSV files  
...
"""
    ...
    use_demo = st.checkbox("Use built-in demo data instead of uploading CSV files", value=False)
    ...
    if use_demo:
        ...
        file_y = st.file_uploader("Upload CSV for Y (dependent variable)", type=["csv"], key="file_y")
        file_x = st.file_uploader("Upload CSV for X (independent variable)", type=["csv"], key="file_x")

        if file_y is None or file_x is None:
            st.warning("Please upload both CSV files or enable demo mode.")
            return

        y, y_col = load_series(file_y, "Y")
        x, x_col = load_series(file_x, "X")
